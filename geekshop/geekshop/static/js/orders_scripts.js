window.onload = function () {
    ...
    $('.order_form').on(types: 'click', selector: 'input[type=checkbox]', data:function(){
        var target = event.target;
        orderitem_num = parseInt(target.name.replace('orderitems-', '').replace('-quantity'));
        if (target.checked) {
           ...
           $.ajax( url: {
               ...
               success: function(data){
                if(data.price){
                    ...
                    var price_html = '<span class="orderitems-'+ orderitem_num + '-price">' + data.price.toString().replace(searchValue: '.', replaceValue: ',') + '</span>' руб;
                    var cur_tr = $('.order_form table').find(selector:'tr:eq(' + (orderitem_num + 1) + ')');
                    cur_tr.find(selector: 'td:eq(2)').html(price_html);
                }
    ...
    function orderSummaryRecalc() {
       ...
       $('.order_total_cost').html(order_total_cost.toFixed(fractionDigits: 2).toString());
    }
